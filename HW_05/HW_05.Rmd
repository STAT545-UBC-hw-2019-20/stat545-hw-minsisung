...
To drop all the data of the Oceania from the gapminder, We can use `filter()` and drop the unused level by `droplevels()`:
...
```{r}
ggsave(here::here("HW_05","gdpPercap_each_continent.png"), plot = new)
```
...
![Loading the exported image](gdpPercap_each_continent.png)
